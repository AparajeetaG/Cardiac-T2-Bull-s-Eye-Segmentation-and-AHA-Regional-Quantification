...
%% -------------------- AHA segmentation --------------------
optsAHA = struct();
optsAHA.clockwise = true;      % if QA looks mirrored, try false on next run
...
for si = 1:num_slice
    ...
    title(ax, sprintf('%s (orig slice %d)', upper(slice_labels{si}), selSlices(si)), 'FontSize', 13, 'FontWeight', 'bold');
end
sgtitle(sprintf('AHA QA overlay | phase %d', phaseIdx), 'FontWeight','bold', 'FontSize', 16);
annotation(figQA, 'textbox', [0.25 0.01 0.5 0.04], 'String', 'Magnitude images with AHA segment labels and myocardial ROI overlay', ...
    'EdgeColor', 'none', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'FontSize', 12);
...
function [Segmentpix, stats, Mask_index_local, Mask_index_global, qa_info, report_file] = ...
    ...
    for m = 1:nSlices
        ...
        % Final AHA remapping. If the QA overlay looks rotated or mirrored,
        % update opts.clockwise first; if still needed, shift these arrays.
        raw_to_global = globalLabels;
