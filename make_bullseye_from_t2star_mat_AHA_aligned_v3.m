...
for si = 1:num_slice
    ...
    title(ax, sprintf('%s (orig slice %d)', upper(slice_labels{si}), selSlices(si)), 'FontSize', 12, 'FontWeight', 'bold');
end
sg = sgtitle(sprintf('AHA QA overlay | phase %d', phaseIdx), 'FontWeight','bold', 'FontSize', 16);
set(sg, 'Units', 'normalized');
sg.Position(2) = 1.06;
annotation(figQA, 'textbox', [0.18 -0.005 0.64 0.035], 'String', 'Magnitude images with myocardial ROI overlay and AHA segment labels', ...
