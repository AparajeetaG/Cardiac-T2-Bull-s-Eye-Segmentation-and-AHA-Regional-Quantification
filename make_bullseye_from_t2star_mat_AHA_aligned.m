...
tl = tiledlayout(1,3,'TileSpacing','compact','Padding','compact');for si = 1:num_slice
    ax = nexttile(tl, si);
...
title(tl, sprintf('AHA QA overlay | phase %d', phaseIdx));
sgtitle(sprintf('AHA QA overlay | phase %d', phaseIdx), 'FontWeight','bold', 'FontSize', 16);% Bulls-eye figure
...
function overlay_aha_labels(ax, maskIndexSlice, sliceType)
    ...
    for k = labels(:)'
        ...
        if any(BW(:))
            ...
            text(ax, c(1), c(2), txt, 'Color','w', 'FontSize',10, 'FontWeight','bold', ...
