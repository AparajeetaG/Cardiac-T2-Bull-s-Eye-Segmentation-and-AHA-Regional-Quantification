...
for si = 1:num_slice
    ...
    colormap(ax, gray);
    overlay_segment_boundaries(ax, Mask_index_global(:,:,si));
...
function overlay_segment_boundaries(ax, labelMap)
    % Draw subtle segment boundaries as thin dark-gray contour lines so they
    % remain visible without covering the insertion point or segment labels.
    B = segment_boundary_map(labelMap);
    if any(B(:))
        contour(ax, double(B), [0.5 0.5], 'Color', [0.15 0.15 0.15], 'LineWidth', 0.5);
    end
end

function B = segment_boundary_map(L)
    L = double(L);
    B = false(size(L));
    % horizontal neighbor differences
    D = (L(:,1:end-1) ~= L(:,2:end)) & (L(:,1:end-1) > 0 | L(:,2:end) > 0);
    B(:,1:end-1) = B(:,1:end-1) | D;
    B(:,2:end)   = B(:,2:end)   | D;
    % vertical neighbor differences
    D = (L(1:end-1,:) ~= L(2:end,:)) & (L(1:end-1,:) > 0 | L(2:end,:) > 0);
    B(1:end-1,:) = B(1:end-1,:) | D;
    B(2:end,:)   = B(2:end,:)   | D;
    % outer boundary of labeled myocardium
    BW = L > 0;
    if any(BW(:))
        B = B | bwperim(BW, 8);
    end
